...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname |invaders-abs(2)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; A Game is a (make-game Rows Base Shots Shots)
(define-struct game (rows base shots-up shots-down))
...
;; An Rows is is one of:
;; - '()
;; - (cons InvaderRow Rows)
;; Interp: a collection of Rows
...
(define (game-main i)
  (big-bang (make-game RS0 (make-base i LIVES0) '() '())
            [to-draw game-draw]
            [on-tick game-tock]
            [on-key game-key]
            [stop-when game-over?]))
...
(define INVS1
  (list (make-invader (* 0 INV-WIDTH+GAP) 30 D0 (make-posn 30 40))
        (make-invader (* 1 INV-WIDTH+GAP) 30 D0 (make-posn 20 50))
        (make-invader (* 2 INV-WIDTH+GAP) 30 D0 (make-posn 10 60))
        (make-invader (* 3 INV-WIDTH+GAP) 30 D0 (make-posn 40 50))))
...
(define R1
  (make-row (first INVS1)
            (first (reverse INVS1)) ; last element
            INVS1))
...
;; t-rows : Invaders -> Rows
;; Turn invaders into rows for testing
(define (t-rows i)
  (cond[(empty? i) '()]
       [(cons? i)
        (list (t-row (first i))
...
(check-expect (game-tock (make-game RS0 B0 '() '()))
              (make-game (row-tock RS0) B0 '() '()))
...
(check-expect (game-kill (make-game RS0 B0 '() (list (make-posn 0 SCN-HEIGHT))))
              (make-game RS0 (base-kill B0) '() '()))
(define (game-kill g)
  (make-game (game-rows g)
...
(check-expect (game-continue (make-game RS0 B0 (list (base->shot B0)) (list S0)))
              (make-game (rows-tock RS0) B0
                         (list (shot-up-tock (base->shot B0)))
...
(define (game-continue g)
  (make-game (rows-tock&remove (game-rows g) (game-shots-up g))
             (game-base g)
             (shots-up-tock&removes (game-shots-up g) (game-rows g))
             (shots-downs-shoot&tock (game-shots-down g) (game-rows g))))
...
(define (remove-all-over-top shots)
  (local [(define (not-shot-over-top? s)
            (not (shot-over-top? s)))]
    (filter not-shot-over-top? shots)))
...
(define (remove-all-below-bot shots)
  (local [(define (not-shot-below-bot? s)
            (not (shot-below-bot? s)))]
    (filter not-shot-below-bot? shots)))
...
(define (shots-remove-hit shots r)
  (local [(define (not-shot-hits-row? s)
            (not (shot-hits-row? s r)))]
    (filter not-shot-hits-row? shots)))
...
(define (any-shots-hit-invader? shots inv)
  (local [ ; shot -> Boolean
          ; Tells if a given shot hits an invader
          (define (shot-hits? s)
            (shot-hits-invader? s inv))]
    (ormap shot-hits? shots)))
...
(define (any-shots-hit-base? shots base)
  (local [ ; shot -> Boolean
          ; Tells if a given shot hits an base
          (define (shot-hits? s)
            (shot-hits-base? s base))]
    (ormap shot-hits? shots)))
...
(define (shots-up-draw-on shots img)
  (foldr shot-up-draw-on img shots))
...
(define (shots-down-draw-on shots img)
  (foldr shot-down-draw-on img shots))
...
(define (rows-template r)
  (cond [(empty? r) ...]
        [(cons? r)
         (... (row-template (first r)) 
              ...
              (rows-template (rest r))
              ...)]))


;; rows-draw-on : Rows Image -> Image
;; Draw the rows on the image

(check-expect (rows-draw-on RS0 MT-SCN)
              (row-draw-on R0 (row-draw-on R1  MT-SCN)))

(define (rows-draw-on r img)
  (cond [(empty? r) img]
        [(cons? r)
         (row-draw-on (first r) (rows-draw-on (rest r) img))]))

;; rows-tock&remove : Rows Shots -> Rows
;; Remove any shot invaders, tock

(check-expect (rows-tock&remove RS0 (cons (make-posn 0 0)
                                          (cons (make-posn 20 20) '())))
              (cons (rows-tock&remove R0 (cons (make-posn 0 0)
                                          (cons (make-posn 20 20) '())))
                    (cons (rows-tock&remove R1 (cons (make-posn 0 0)
                                          (cons (make-posn 20 20) '()))) '())))

(define (rows-tock&remove r s)
  (cond [(empty? r) '()]
        [(cons? r)
         (cons (row-tock&remove (first r) s)
              (rows-tock&remove (rest r) s))]))

;; rows-tock : Rows -> Rows
;; Advance the invaders, countdown the timers

(check-expect (rows-tock RS0)
              (cons (row-tock R0) (row-tock R1) '()))

(define (rows-tock r)
  (cond [(empty? r) '()]
        [(cons? r)
         (cons (row-tock (first r)) 
              (rows-tock (rest r)))]))


;; shots-up-tock&removes: Shots Rows -> Shots
;; Remove any shots hitting row, tock upward

(define (shots-up-tock&removes s r)
  (cond [(empty? r) s]
        [(cons? r)
         (shots-up-tock&removes (shots-up-tock&remove s (first r)) (rest r))]))

;; shots-downs-shoot&tock : Shots Rows -> Shots
;; Tock downard, add any shots from ready invaders in rows

(define (shots-downs-shoot&tock s r)
  (cond [(empty? r) s]
        [(cons? r)
         (shots-downs-shoot&tock (shots-down-shoot&tock s (first r)) (rest r))]))
...
;; row-tock : Row -> Row
;; Advance the invaders, countdown the timers
(check-expect (row-tock (t-row INV0))
              (row-countdown (row-march (t-row INV0))))
...
(check-expect (row-draw-on (t-row INV0) A-SCN)
...
(check-expect (invaders-flip&down (list INVR))
              (list (invader-flip&down INVR)))

(define (invaders-flip&down is)
  (map invader-flip&down is))
...
(define (invaders-shoot is s)
  (foldr invader-shoot s is))

;; invaders-countdown : LoI -> LoI
;; Countdown all the timers
(check-expect (invaders-countdown '()) '())
(check-expect (invaders-countdown (list INV0))
              (list (invader-countdown INV0)))

(define (invaders-countdown is)
  (map invader-countdown is))
...
(define (invaders-remove-shot is shots)
  (local [(define (not-any-shots-hit-invader? i)
            (not (any-shots-hit-invader? shots i)))]
    (filter not-any-shots-hit-invader? is)))         
...
(define (invaders-hit-shot? is shot)
  (local [(define (inv-hit? i)
            (shot-hits-invader? shot i))]
    (ormap inv-hit? is)))
